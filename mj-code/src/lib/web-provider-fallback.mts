...
export class FallbackSearchProvider {
  ...
  async search(input: {
    ...
    // Strategy 2: Try DuckDuckGo Instant Answer API (lite JSON)
    try {
...
export function parseFallbackHtml(
  ...
  // Strategy 2: Broader link extraction if Strategy 1 found nothing
  if (results.length === 0) {
    // Try matching any links with meaningful hrefs (duckduckgo redirect URLs)
