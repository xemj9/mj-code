...
export function resolveInteractiveShellAnswer(
  ...
  // - If the interactive palette is visible with a selected command, use that selection
  // - Otherwise, return "/" so the REPL handler renders the text palette fallback
  if (trimmedAnswer === "/") {
    if (paletteState?.visible && paletteState.selectedCommand) {
