...
export class ContextManager {
  ...
  compact(messages: MessageLike[]): CompactResult {
    ...
    const chunkSummary = summarizeMessages(chunk);
    this.rollingSummary = mergeSummaries(this.rollingSummary, chunkSummary, 800);
    this.lastPlan = {
      ...(this.lastPlan ?? {
...
function mergeSummaries(existingSummary: string, nextSummary: string, budgetTokens: number): string {
  if (!existingSummary) {
    return abbreviate(nextSummary, budgetTokens * 4);
  }
  if (!nextSummary) {
    return abbreviate(existingSummary, budgetTokens * 4);
  }

  // If the existing summary is already near the budget, compress it first
  const charBudget = budgetTokens * 4;
  const existingLines = existingSummary.split("\n").filter(Boolean);
  const nextLines = nextSummary.split("\n").filter(Boolean);

  // Keep recent turns in more detail, older turns get compressed
  const combined = [...existingLines, ...nextLines];
  if (combined.join("\n").length <= charBudget) {
    return combined.join("\n");
  }

  // Progressive compression: keep recent turns verbatim, progressively compress older ones
  const result: string[] = [];
  ...
  for (let i = combined.length - 1; i >= 0; i--) {
    ...
    if (line.length + 1 <= remaining) {
      ...
      // Try to compress this line
      const compressed = compressSummaryLine(line, Math.max(40, remaining - 20));
      if (compressed && compressed.length + 1 <= remaining) {
        result.unshift(compressed);
        remaining -= compressed.length + 1;
      }
...
/**
 * Compress a single summary line by removing less important details.
 */
function compressSummaryLine(line: string, maxLen: number): string | null {
  if (line.length <= maxLen) {
    return line;
  }

  // For tool call lines, strip the input details
  const callMatch = line.match(/^(call \w+\()\s*/);
  if (callMatch) {
    return abbreviate(line, maxLen);
  }

  // For tool result lines, keep just the tool name and status
  const toolMatch = line.match(/^(tool_(?:ok|err))\s*=>/);
  if (toolMatch) {
    return abbreviate(line, maxLen);
  }
...
function summarizeContent(content: unknown): string {
  if (content && typeof content === "object") {
    try {
      return abbreviate(JSON.stringify(content), 300);
    } catch {
      return "";
    }
  }
  ...
  // Native tool result format: JSON with ok/result/error
  if (text.startsWith("{") && text.includes('"ok"')) {
    try {
      const parsed = JSON.parse(text) as { ok?: boolean; result?: unknown; error?: string };
      if (parsed.ok && parsed.result) {
        const resultSummary = summarizeToolResultValue(parsed.result);
        return `tool_ok => ${abbreviate(resultSummary, 280)}`;
      }
      if (!parsed.ok && parsed.error) {
        return `tool_err => ${abbreviate(parsed.error, 280)}`;
      }
    } catch {
      // fall through
    }
  ...
  // JSON protocol tool result format
  if (text.startsWith('Tool result for "')) {
    const toolName = text.match(/Tool result for "([^"]+)"/)?.[1] ?? "unknown";
    // Extract meaningful info from the result body
    const bodyStart = text.indexOf("\n");
    const body = bodyStart >= 0 ? text.slice(bodyStart + 1).trim() : "";
    if (body) {
      return `${toolName} => ${abbreviate(body, 280)}`;
    }
    return `${toolName} => (executed)`;
  }

  // For assistant messages that contain tool calls (JSON protocol)
  if (text.includes('"type":"tool_call"') || text.includes('"type": "tool_call"')) {
    try {
      const parsed = JSON.parse(text) as { type: string; tool: string; input?: unknown };
      if (parsed.tool) {
        const inputSummary = summarizeToolInputValue(parsed.input);
        return `call ${parsed.tool}(${abbreviate(inputSummary, 200)})`;
      }
    } catch {
      // fall through
    }
...
function summarizeToolResultValue(result: unknown): string {
  ...
  if (typeof r.path === "string") {
    ...
    const size = r.bytesWritten ? ` ${r.bytesWritten} bytes` : "";
    return `${r.path}${lines}${size}`;
  }
  // Directory listing
  ...
  if (Array.isArray(r.results)) {
    return `${(r.results as unknown[]).length} web results`;
  }
  // Shell job
  if (r.jobId) {
    return `job ${r.jobId} status=${r.status ?? "unknown"}`;
  }
  // Generic
  return abbreviate(JSON.stringify(r), 200);
}

function summarizeToolInputValue(input: unknown): string {
  if (!input || typeof input !== "object") {
    return `${input ?? ""}`;
  }
  const r = input as Record<string, unknown>;
  const keys = ["path", "query", "url", "command", "directory", "pattern", "content"];
  for (const key of keys) {
    if (typeof r[key] === "string" && (r[key] as string).length > 0) {
      return `${key}=${abbreviate(r[key] as string, 150)}`;
    }
  }
  return abbreviate(JSON.stringify(r), 150);
}
