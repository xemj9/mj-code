...
export function createSlashPaletteController(options: SlashPaletteControllerOptions = {}) {
  ...
  async function syncReport(line: string, continuationLine: string | null = null) {
    ...
    // Fast path: when no picker resolver is set, we can update the palette synchronously
    // This makes the command palette appear instantly when the user types "/"
    if (!resolvePicker) {
      applyPaletteReport(normalizedLine, effectiveLine, null, requestId);
      return;
    }

    const pickerReport = await resolvePicker(effectiveLine);
    if (requestId !== state.requestId) {
      return;
    }
    applyPaletteReport(normalizedLine, effectiveLine, pickerReport, requestId);
  }

  function applyPaletteReport(
    normalizedLine: string,
    effectiveLine: string,
    pickerReport: PaletteReport | null,
    requestId: number,
  ): void {
    if (requestId !== state.requestId) {
      return;
