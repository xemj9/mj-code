...
function evaluateDocsOnlyAllowance(
  ...
  // Academic and research sites are always allowed (even in docs-only mode)
  if (ACADEMIC_HOSTS.has(metadata.domain)) {
    return {
      allowed: true,
      reason: null,
      matchedAllowDomain: null,
  ...
  // Knowledge and documentation hosting sites are always allowed
  if (KNOWLEDGE_HOSTS.has(metadata.domain)) {
    return {
      allowed: true,
      reason: null,
      matchedAllowDomain: null,
  ...
  // Also allow subdomains of academic and knowledge hosts
  if (metadata.domain) {
    for (const host of ACADEMIC_HOSTS) {
      if (metadata.domain.endsWith(`.${host}`)) {
        return {
          allowed: true,
          reason: null,
          matchedAllowDomain: null,
