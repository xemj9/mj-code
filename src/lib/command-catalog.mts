...
const REPL_PALETTE_ENTRIES: PaletteSeed[] = [
  ...
  },
  {
    ...
    section: "advanced",
    featured: true,
    keywords: ["search", "web", "internet", "google", "find", "lookup"],
  },
  {
    command: "/fetch",
    label: "/fetch",
    description: "Fetch a URL with policy checks and content type validation.",
    category: "advanced_debug",
    section: "advanced",
    keywords: ["fetch", "url", "download", "http", "request"],
  },
  {
    command: "/extract",
    label: "/extract",
    description: "Fetch a URL and extract readable content from the page.",
    category: "advanced_debug",
    section: "advanced",
    keywords: ["extract", "readable", "content", "article", "parse"],
  },
];
...
function createPaletteSection(
  ...
  const entries = REPL_PALETTE_ENTRIES
    ...
    .slice(0, category === "core" || category === "session_history_resume" || category === "advanced_debug" ? 4 : 3);
  return {
    category,
