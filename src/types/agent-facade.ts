...
export interface AgentTerminalUi {
  ...
  close(): void;
  // Enhanced UI extensions — used by cli-runtime and agent-turn-engine
  printAssistant?(content: string): void;
  printUserMessage?(content: string): void;
  printTokenUsage?(inputTokens: number, outputTokens: number): void;
  printEffortLevel?(level: "low" | "medium" | "high" | "max"): void;
  printContextBar?(used: number, max: number): void;
  beginAssistantStream?(): unknown;
  ...
  printChangePreview?(changeSet: Record<string, unknown>): void;
  printWarning?(message: string): void;
  confirmAction?(approvalContext: unknown): Promise<boolean>;
  confirm?(message: string): Promise<boolean>;
  printProviderFailure?(details: Record<string, unknown>): void;
  printProviderEvent?(event: Record<string, unknown>): void;
  printLocalContextPrefetch?(result: Record<string, unknown>): void;
  printSection?(title: string, body: string): void;
  printAbout?(): void;
