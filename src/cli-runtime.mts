...
export async function main(): Promise<void> {
  ...
  const ui = createEnhancedTerminalUi();
  ...
  try {
    ...
    if (command === "search") {
      ...
      try {
        const result = await agent.invokeCommandTool("web_search", { query }) as Record<string, unknown>;
        console.log(renderWebSearchResult(query, result));
      } catch (error) {
        console.error(error instanceof Error ? error.message : String(error));
        process.exitCode = 1;
      }
    ...
    if (command === "fetch") {
      ...
      try {
        const result = await agent.invokeCommandTool("fetch_url", { url }) as Record<string, unknown>;
        console.log(renderWebFetchResult(url, result));
      } catch (error) {
        console.error(error instanceof Error ? error.message : String(error));
        process.exitCode = 1;
      }
    ...
    if (command === "extract") {
      ...
      try {
        const result = await agent.invokeCommandTool("extract_content", { url }) as Record<string, unknown>;
        console.log(renderWebExtractResult(url, result));
      } catch (error) {
        console.error(error instanceof Error ? error.message : String(error));
        process.exitCode = 1;
      }
...
async function runInteractiveSession(agent: CliAgent): Promise<void> {
  ...
  // Auto-suggest session resume if there's a recent session
  await suggestSessionResume(agent);
  ...
  if (batchLines) {
    for await (const rawLine of batchLines) {
      ...
      try {
        if (agent.ui.printUserMessage) {
          agent.ui.printUserMessage(input);
        }
        ...
        if (agent.ui.printTokenUsage) {
          const usage = agent.getUsageSummary() as Record<string, unknown> | null;
          if (usage) {
            ...
            agent.ui.printTokenUsage(promptTokens, completionTokens);
          }
        }
      } catch (error) {
  ...
  while (true) {
    ...
    try {
      ...
      // Show token usage after each turn
      if (typeof (agent.ui as unknown as Record<string, unknown>).printTokenUsage === "function") {
        const usage = agent.getUsageSummary() as Record<string, unknown> | null;
        if (usage) {
          const promptTokens = Number(usage.promptTokens ?? usage.prompt_tokens ?? 0);
          const completionTokens = Number(usage.completionTokens ?? usage.completion_tokens ?? 0);
          ((agent.ui as unknown as Record<string, unknown>).printTokenUsage as (i: number, o: number) => void)(promptTokens, completionTokens);
        }
      }
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
...
async function handleSlashCommand(input: string, agent: CliAgent): Promise<boolean> {
  ...
  switch (command) {
    case "/":
      // The interactive palette overlay is handled by the palette controller
      // when the user types "/" in the readline. If we reach here (user pressed
      // Enter with just "/"), render a beautiful full command palette panel.
      console.log(renderInteractiveCommandPalette(getInteractiveCommandPalette()));
      return true;
    case "/help":
    ...
    case "/capabilities":
      ...
      return true;
    case "/config": {
      const configSub = parts[0];
      if (!configSub) {
        // Show key config values in a readable format
        const cfg = redactConfig(agent.config) as Record<string, unknown>;
        console.log(`  Provider:  ${cfg.provider ?? "unknown"}`);
        console.log(`  Model:     ${cfg.model ?? "auto"}`);
        console.log(`  CWD:       ${cfg.cwd ?? process.cwd()}`);
        console.log(`  Permission: ${cfg.permissionMode ?? "default"}`);
        console.log(`  Approval:  ${cfg.approvalPolicy ?? "ask"}`);
        console.log(`  Network:   ${cfg.networkMode ?? "off"}`);
        console.log(`  Stream:    ${cfg.streamOutput ?? true}`);
        console.log(`  MaxTokens: ${cfg.maxTokens ?? "default"}`);
        console.log(`  MaxSteps:  ${cfg.maxSteps ?? "default"}`);
        console.log(`  Effort:    ${getEffortLevel()}`);
        console.log(`  WebProvider: ${cfg.webProvider ?? "fallback"}`);
        return true;
      }
      if (configSub === "raw") {
      ...
      if (configSub === "effort") {
        const level = parts[1];
        if (!level || !isValidEffortLevel(level)) {
          console.log(`Current effort: ${getEffortLevel()}`);
          console.log("Usage: /config effort <low|medium|high|max>");
          return true;
        }
        setEffortLevel(level);
        if (typeof (agent.ui as unknown as Record<string, unknown>).printEffortLevel === "function") {
          ((agent.ui as unknown as Record<string, unknown>).printEffortLevel as (l: "low" | "medium" | "high" | "max") => void)(level as "low" | "medium" | "high" | "max");
        }
        return true;
      }
      console.log("Usage: /config [raw|effort <level>]");
      return true;
    }
    case "/status":
      try {
      ...
      return true;
    case "/model": {
      const modelSub = parts[0];
      if (!modelSub) {
        // Show current model decision
        const decision = agent.getModelDecision() as unknown as Record<string, unknown> | null;
        if (decision) {
          console.log(`  Model:    ${decision.chosenModel ?? agent.config.model ?? "unknown"}`);
          console.log(`  Provider: ${decision.chosenProvider ?? agent.config.provider ?? "unknown"}`);
          console.log(`  Reason:   ${decision.reason ?? "default"}`);
          if (decision.fallbackModels && Array.isArray(decision.fallbackModels)) {
            console.log(`  Fallbacks: ${decision.fallbackModels.join(", ")}`);
          }
        } else {
          console.log(`  Model:    ${agent.config.model ?? "unknown"}`);
          console.log(`  Provider: ${agent.config.provider ?? "unknown"}`);
        }
        return true;
      }
      if (modelSub === "list") {
        try {
          const models = await agent.listModels() as string[];
          if (models.length === 0) {
            agent.ui.printInfo("model", "No models available from provider.");
          } else {
            console.log("Available models:");
            for (const model of models) {
              const current = model === agent.config.model ? " ← current" : "";
              console.log(`  - ${model}${current}`);
            }
          }
        } catch (error) {
          agent.ui.printError(`Failed to list models: ${error instanceof Error ? error.message : String(error)}`);
        }
        return true;
      }
      // Set model: /model <model-name>
      const previousModel = agent.config.model;
      (agent.config as Record<string, unknown>).model = modelSub;
      agent.ui.printInfo("model", `Model changed: ${previousModel ?? "auto"} → ${modelSub}`);
      return true;
    }
    case "/provider":
      console.log(JSON.stringify(agent.getProviderDecision(), null, 2));
    ...
    case "/network-mode": {
      ...
      if (!modeArg) {
        const current = agent.getNetworkMode() as Record<string, unknown>;
        console.log(`  Current: ${current.mode ?? current.networkMode ?? "unknown"}`);
        console.log("Usage: /network-mode <off|docs-only|open-web>");
        return true;
      }
      const validModes = ["off", "docs-only", "open-web"];
      if (!validModes.includes(modeArg)) {
        agent.ui.printError(`Invalid network mode "${modeArg}". Valid: off, docs-only, open-web`);
        return true;
      }
      const previous = (agent.config as Record<string, unknown>).networkMode;
      (agent.config as Record<string, unknown>).networkMode = modeArg;
      agent.ui.printInfo("network", `Network mode: ${previous ?? "unknown"} → ${modeArg}`);
      return true;
    }
    case "/mcp":
    ...
    case "/compact": {
      ...
      if (result.compactedMessages > 0) {
        ...
        const summaryPreview = result.rollingSummary.length > 200
          ? `${result.rollingSummary.slice(0, 200)}…`
          : result.rollingSummary;
        agent.ui.printInfo("summary", summaryPreview);
      } else {
        agent.ui.printInfo("compact", "Nothing to compact yet.");
      }
      return true;
    }
    case "/cost":
...
async function handleMcpCommand(parts: string[], agent: CliAgent): Promise<boolean> {
  ...
  if (subcommand === "servers") {
    const servers = agent.getMcpServers() as Array<Record<string, unknown>>;
    if (!servers || servers.length === 0) {
      agent.ui.printInfo("mcp", "No MCP servers configured. Add servers to ~/.mj-code/mcp.json or .mcp.json");
      return true;
    }
    for (const server of servers) {
      ...
      const statusIcon = status === "ready" ? "🟢" : status === "degraded" ? "🟡" : status === "failed" ? "🔴" : "⚪";
      console.log(`  ${statusIcon} ${server.id ?? "?"} (${server.name ?? "?"}) status=${status} health=${health} tools=${toolCount} latency=${latency}`);
    }
    return true;
  }

  if (subcommand === "tools") {
    const tools = agent.getMcpTools() as Array<Record<string, unknown>>;
    if (!tools || tools.length === 0) {
      agent.ui.printInfo("mcp", "No MCP tools available. Check that your MCP servers are running.");
      return true;
    }
    for (const tool of tools) {
      const serverId = tool.serverId ?? "?";
      const readonly = (tool.annotations as Record<string, unknown>)?.readOnlyHint ? " [readonly]" : "";
      console.log(`  🔧 ${tool.normalizedName ?? tool.name} (${serverId})${readonly}`);
      if (tool.description) {
        console.log(`     ${String(tool.description).slice(0, 100)}`);
      }
    }
    return true;
  }
  ...
  if (subcommand === "inspect") {
    ...
    const info = agent.inspectMcpServer(serverId) as Record<string, unknown> | null;
    if (!info) {
      agent.ui.printError(`MCP server "${serverId}" not found.`);
      return true;
    }
    const clientView = info.client as Record<string, unknown> | undefined;
    const toolList = info.tools as Array<Record<string, unknown>> | undefined;
    console.log(`  Server: ${info.name ?? serverId}`);
    console.log(`  Status: ${clientView?.status ?? "unknown"}`);
    console.log(`  Health: ${clientView?.healthScore ?? "?"}`);
    console.log(`  Protocol: ${clientView?.protocolVersion ?? "unknown"}`);
    console.log(`  Latency: ${clientView?.latencyMs != null ? `${clientView.latencyMs}ms` : "—"}`);
    console.log(`  Tools: ${toolList?.length ?? 0}`);
    if (toolList && toolList.length > 0) {
      for (const tool of toolList.slice(0, 10)) {
        console.log(`    - ${tool.normalizedName ?? tool.name}`);
      }
      if (toolList.length > 10) {
        console.log(`    ... ${toolList.length - 10} more`);
      }
    }
    if (info.command) {
      console.log(`  Command: ${info.command} ${(info.args as string[] ?? []).join(" ")}`);
    }
    return true;
  }
  ...
  if (subcommand === "test") {
    ...
    agent.ui.printInfo("mcp", `Testing ${serverId}...`);
    try {
      const result = await agent.testMcpServer(serverId) as Record<string, unknown>;
      const server = result.server as Record<string, unknown> | undefined;
      const ping = result.ping as Record<string, unknown> | undefined;
      console.log(`  ✓ Connected: ${server?.status ?? "unknown"}`);
      console.log(`  ✓ Ping: ${ping?.latencyMs ?? "?"}ms`);
      console.log(`  ✓ Tools: ${result.toolCount ?? 0}`);
    } catch (error) {
      agent.ui.printError(`MCP test failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    return true;
  }

  if (subcommand === "refresh") {
    agent.ui.printInfo("mcp", "Refreshing MCP tool registry...");
    try {
      // Re-initialize the MCP registry to pick up new tools
      const servers = agent.getMcpServers() as Array<Record<string, unknown>>;
      for (const server of servers) {
        try {
          await agent.testMcpServer(server.id as string);
        } catch {
          // Skip failed servers
        }
      }
      agent.ui.printInfo("mcp", "MCP registry refreshed.");
    } catch (error) {
      agent.ui.printError(`Refresh failed: ${error instanceof Error ? error.message : String(error)}`);
    }
    return true;
  }

  console.log("Usage:");
  console.log("/mcp servers              List MCP servers and their status");
  console.log("/mcp tools                List available MCP tools");
  console.log("/mcp inspect <server-id>  Inspect a specific MCP server");
  console.log("/mcp test <server-id>     Test connection to an MCP server");
  console.log("/mcp refresh              Refresh MCP tool registry");
  return true;
}
...
async function handleUndoCommand(parts: string[], agent: CliAgent): Promise<boolean> {
  ...
  try {
    const result = await agent.undoChange(changeSetId) as Record<string, unknown>;
    if (result && typeof result === "object") {
      const files = Array.isArray(result.restoredFiles) ? result.restoredFiles : [];
      if (files.length > 0) {
        agent.ui.printInfo("undo", `Restored ${files.length} file(s): ${files.slice(0, 3).join(", ")}${files.length > 3 ? ` +${files.length - 3} more` : ""}`);
      } else {
        agent.ui.printInfo("undo", "Undo completed.");
      }
    } else {
      agent.ui.printInfo("undo", "No changes to undo.");
    }
  } catch (error) {
    agent.ui.printError(error instanceof Error ? error.message : String(error));
  }
  return true;
}
...
// ─── Web Search/Fetch/Extract Renderers ───────────────────────

function renderWebSearchResult(query: string, result: Record<string, unknown>): string {
  const lines: string[] = [];
  ...
  if (!results || results.length === 0) {
    // Try to extract from the raw result object
  ...
  for (let i = 0; i < results.length; i++) {
    ...
    const url = String(entry.url ?? entry.link ?? entry.canonicalUrl ?? "");
    const snippet = String(entry.snippet ?? entry.description ?? entry.text ?? "");
...
function renderWebFetchResult(url: string, result: Record<string, unknown>): string {
  const lines: string[] = [];
  ...
  if (typeof content === "string" && content.length > 0) {
    const truncated = content.length > 2000 ? `${content.slice(0, 2000)}…\n\n  [truncated — ${content.length} total characters]` : content;
    lines.push(`  Content (${content.length} chars):\n`);
    lines.push(truncated);
  } else if (result?.status || result?.statusCode) {
    lines.push(`  Status: ${result.status ?? result.statusCode}`);
...
function renderWebExtractResult(url: string, result: Record<string, unknown>): string {
  const lines: string[] = [];

  lines.push(`\n  📄 Extract: ${url}`);

  const content = result?.content ?? result?.text ?? result?.extractedContent ?? result?.body;
  if (typeof content === "string" && content.length > 0) {
...
/**
 * Suggest resuming a recent session if one exists.
 * This is inspired by Claude Code's session continuity behavior.
 */
async function suggestSessionResume(agent: CliAgent): Promise<void> {
  try {
    const recommendation = await agent.recommendSessionResume("current") as unknown as Record<string, unknown> | null;
    if (!recommendation) {
      return;
    }

    const sessionId = recommendation.sessionId as string | undefined;
    const confidence = recommendation.confidence as string | undefined;

    // Only suggest if there's a clear recommendation
    if (!sessionId || confidence === "low") {
      return;
    }

    const preview = recommendation.preview as string | undefined;
    const previewText = preview ? ` (${preview.slice(0, 60)})` : "";

    agent.ui.printInfo(
      "resume",
      `Recent session found: ${sessionId.slice(0, 20)}…${previewText}`,
    );
    agent.ui.printInfo(
      "hint",
      `Type /resume ${sessionId.slice(0, 12)} to continue, or start a new conversation.`,
    );
  } catch {
    // Don't block startup if session resume check fails
  }
}
