...
export async function fetchUrl(
  ...
  // Prefer extracted readable text over raw HTML for bodyPreview
  // This avoids returning large amounts of HTML markup that gets truncated
  // before the useful content appears
  const bodyPreview = extracted
    ? previewText(extracted.readableText, context.maxOutputChars)
    : previewText(response.content, context.maxOutputChars);
  ...
  return {
    ...
    extractedMeta: extracted
      ...
      : null,
    readableText: extracted?.readableText ?? null,
    sourcePack: summarizeSourcePack(sourcePack),
    citations: sourcePack.citations,
  };
}
...
async function maybeCheckRobots(
  ...
  // Skip robots.txt check in open-web mode for well-known academic and documentation sites
  // These sites often have restrictive robots.txt but allow content access
  const ACADEMIC_AND_DOC_DOMAINS = [
    "arxiv.org",
    "scholar.google.com",
    "dl.acm.org",
    "ieeexplore.ieee.org",
  ...
  if (context.networkMode === "open-web" && metadata.domain != null) {
    const domain = metadata.domain;
    const isAcademicOrDoc = ACADEMIC_AND_DOC_DOMAINS.some((d) =>
      domain === d || domain.endsWith(`.${d}`)
    );
    if (isAcademicOrDoc) {
      return; // Skip robots.txt for these sites in open-web mode
    }
  }
  ...
  try {
    ...
    // Any other error fetching robots.txt should NOT block the actual request
...
function isBlockedByRobots(robotsText: string, pathname: string | null): boolean {
  ...
  for (const rawLine of lines) {
    ...
    if (rawKey === "disallow" && value) {
      disallowRules.push(value);
    }

    if (rawKey === "allow" && value) {
  ...
  for (const rule of allowRules) {
    if (effectivePath.startsWith(rule)) {
      return false;
    }
  }

  // Only block if the disallow rule is very specific (longer than 3 chars)
  // to avoid blocking by broad patterns like "/" or "/search" which
  // are common but don't apply to programmatic access for content extraction
  return disallowRules.some((rule) => {
    if (rule.length <= 1) return false; // Skip "/" — too broad
    if (rule === "/search" || rule === "/Search") return false; // Skip common search paths
    return effectivePath.startsWith(rule);
  });
}
