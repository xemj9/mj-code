...
interface OpenAiChatCompletionPayload {
  choices?: Array<{
    message?: {
      ...
      tool_call?: OpenAiResponseToolCall | null;
    } | null;
  }>;
...
export class OpenAiCompatibleProvider {
  ...
  async complete({
    ...
    const rawToolCalls = payload.choices?.[0]?.message?.tool_calls
      ?? (payload.choices?.[0]?.message?.tool_call ? [payload.choices[0].message.tool_call] : null);
    const toolCalls = normalizeToolCalls(rawToolCalls);

    // Combine reasoning and content for thinking models like GLM-5
    // Prefer the main content when available; only fall back to reasoning
    // when the model produced no formal content (e.g., it only thought)
    const mainText = extractText(content);
    const reasoning = typeof reasoningContent === "string" && reasoningContent.trim()
      ? reasoningContent.trim()
      : "";

    // If there are tool calls, ignore reasoning entirely — tool calls are
    // the primary output and reasoning might confuse downstream parsing.
    const shouldIgnoreReasoning = toolCalls.length > 0;

    return {
      text: shouldIgnoreReasoning
        ? mainText
        : mainText || reasoning,
  ...
  async completeWithStreaming(
    ...
    const streamedToolCalls = toolCallBuilders
      .filter(Boolean)
      .map((item, index) => normalizeToolCall(item, index))
      .filter((item): item is ProviderToolCall => item != null);

    // If tool calls were produced, prefer main content over reasoning
    // to avoid confusing downstream JSON protocol parsing
    const effectiveText = streamedToolCalls.length > 0
      ? text
      : text || reasoningText;
